...
try
	...
	while (true)
		...
		[x,R3] = strtok(R3);
		[d,R4] = strtok(R3);
		[I(k,1),P(k,1),M(k,1),N(k,1)] = deal(str2double(a)+1,str2double(b),1+str2double(c),str2num(d));
		nn{k} = [];
		tn{k} = [];
		for i=1:N(k)
			[a,R3] = strtok(R4);
			[b,R4] = strtok(R3);
			if (b == '(D)'), nn{k}  = [nn{k}; str2double(a)+1];
...
if verbose
	for i=1:length(I)
		fprintf('[ %i ] %8.6f P= %i N= %i',I(i),P(i),M(i),N(i));
		for j=1:N(i)
...
for i=1:length(I)
    for j=1:N(i)
        if strcmp(char(tn{i}(j)),'(G)'), et = [et; nn{i}(j)]; end;
    end;
end;
...
% we give them the minimum leaf index in their subtree (this is Q)
